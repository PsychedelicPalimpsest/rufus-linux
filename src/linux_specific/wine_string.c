 ...
 */
 ...
 #include "wine_string.h"
 
 #define WINAPI
 
 #define GetThreadLocale() NULL
 #define TRACE(...) {}
 #define WARN(...) {}
 ...
 #define HeapAlloc(_1, _2, SIZE) malloc((SIZE))
 #define HeapFree(_1, _2, PTR_) free(PTR_)
 ...
 int CompareStringW(
     ...
     // Null-checking for both strings
     if (!lpString1 && !lpString2) return 0;  // Both are NULL, considered equal
     if (!lpString1) return -1;  // NULL is considered less than any string
     if (!lpString2) return 1;   // NULL is considered less than any string
 
     int i = 0;
     while (i < cchCount1 && i < cchCount2) {
         wchar_t c1 = lpString1[i];
         wchar_t c2 = lpString2[i];
 
         // If NORM_IGNORECASE flag is set, convert to lowercase for comparison
         if (dwCmpFlags & NORM_IGNORECASE) {
             c1 = towlower(c1);
             c2 = towlower(c2);
         }
 
         if (c1 < c2) return -1;  // str1 is lexicographically smaller
         if (c1 > c2) return 1;   // str1 is lexicographically greater
 
         i++;
     }
 
     // If one string is shorter than the other, the shorter one is considered less
     if (i < cchCount1) return 1;   // lpString1 is longer
     if (i < cchCount2) return -1;  // lpString2 is longer
 ...
 BOOL IsDBCSLeadByteEx(WORD ignored_codepage, wchar_t wc) {
     // Shift-JIS lead byte ranges (assuming the wide char is within 0xFFFF range)
     if ((wc >= 0x81 && wc <= 0x9F) || (wc >= 0xE0 && wc <= 0xFC)) {
         return 1;
     }
     // GBK lead byte ranges (0x81 to 0xFE)
     if (wc >= 0x81 && wc <= 0xFE) {
         return 1;
     }
     // Big5 lead byte ranges (0x81 to 0xFE)
     if (wc >= 0x81 && wc <= 0xFE) {
         return 1;
     }
 
     // Add additional checks for other encodings if needed
 ...
 DWORD CharLowerBuffW(WCHAR *str, DWORD len)
 {
     ...
     {
         str[i] = (WCHAR) towlower((wint_t) str[i]);
     }
     return len;
 }
